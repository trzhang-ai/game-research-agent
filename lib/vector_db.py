...
class VectorStore:
    ...
    def upsert(self, item: Union[Document, Corpus, List[Document]]):
        """
        Upsert documents to the vector store with automatic embedding generation.
        ...
        Args:
            item (Union[Document, Corpus, List[Document]]): Documents to upsert.
                Can be a single Document, a Corpus collection, or a list of Documents.
        ...
        Example:
            >>> store.upsert(Document(content="AI is transforming healthcare"))
            >>> store.upsert([doc1, doc2, doc3])  # Batch upsert
            >>> store.upsert(Corpus([doc1, doc2]))  # Add corpus
        """
        item_dict = self._prepare_item(item)
